...
links <- and_languages %>% 
  ...
  mutate(
    link = str_replace_all(
      paste0("(https://en.wikipedia.org/wiki/", language, "_language)"),
      " ",
      "_"
    ),
    markdown = paste0("* [", language, "]", link),
    markdown = if_else(
      language == "English",
      paste(
        markdown,
        "(with and without [Oxford comma](https://en.wikipedia.org/wiki/Serial_comma))"
      ),
      markdown
    )
  ) %>% 
